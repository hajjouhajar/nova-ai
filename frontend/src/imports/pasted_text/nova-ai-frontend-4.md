...
LE PDF DOIT RESSEMBLER À UN VRAI CERTIFICAT/DIPLÔME PROFESSIONNEL, pas à une
carte d'interface exportée telle quelle. Éléments attendus, au format A4
paysage :
- Une **bordure décorative fine** encadrant toute la page (simple filet ou
  double filet dans la couleur d'accent, style diplôme classique — pas de
  motifs chargés)
- En haut : logo Nova AI centré, petit et discret
- Le titre principal "CERTIFICAT DE RÉUSSITE" en majuscules, grande taille,
  police avec empattements ou semi-formelle (différente de la police
  standard de l'interface, pour marquer le caractère officiel du document)
- Une phrase d'introduction sobre : "Ce certificat est décerné à"
- Le **nom complet de l'utilisateur** en très grande taille, centré, dans
  une police élégante (type police de signature/script discrète, ou
  simplement une grande taille en gras) — c'est l'élément visuel dominant
  de la page
- Une phrase de validation : "pour avoir complété avec succès le parcours"
  suivi du **titre du parcours** en gras
- La **liste des compétences validées**, présentée sobrement (pas en tags
  colorés comme dans l'interface web — plutôt une ligne de texte ou des
  puces discrètes, cohérent avec le style formel du document)
- En bas de page, sur une ligne : à gauche la **date d'émission**, au
  centre une **ligne de signature** (ex. "Nova AI — Plateforme de mentorat
  intelligent" sous une ligne horizontale, simulant une signature
  d'autorité délivrante), à droite le **QR code de vérification** avec le
  numéro unique du certificat en dessous en petite police
- Palette du certificat : fond blanc ou ivoire, texte en gris très foncé/
  noir, une seule couleur d'accent utilisée avec parcimonie (bordure,
  titre, ligne de signature) — jamais de dégradés flashy ni d'icônes
  ludiques sur ce document précis, contrairement au reste de l'app
...
5. EXAMEN FINAL (débloqué uniquement quand TOUS les modules sont à 100%)
   - Écran d'introduction : "Tu as terminé tous les modules ! Passe l'examen
     final pour obtenir ton certificat.", nombre de questions, durée totale
     estimée, et une mention claire "Chaque question est chronométrée
     individuellement — une fois le temps écoulé, la question est validée
     automatiquement avec la réponse sélectionnée jusque-là (ou vide si
     aucune réponse)." Bouton "Commencer l'examen".
   - INTERFACE D'EXAMEN — une question à la fois, avec chronomètre PAR
     QUESTION (pas un chronomètre global unique) :
     - En haut de l'écran : numéro de la question (ex. "Question 4/15"),
       barre de progression fine, et un chronomètre circulaire ou linéaire
       bien visible affichant le temps restant pour CETTE question
       spécifique (ex. 60 secondes pour un QCM, davantage pour une question
       ouverte — la durée par question est définie côté backend selon son
       type/difficulté)
     - Le chronomètre change de couleur à l'approche de la fin (ex. neutre
       → orange sous 15s → rouge sous 5s) pour créer une urgence visuelle
       sans être stressant à l'excès
     - Dès que le temps est écoulé : passage automatique à la question
       suivante, sans possibilité de revenir en arrière (comportement type
       examen réel, pas un quiz réentrant)
     - Bouton "Question suivante" disponible aussi manuellement si
       l'utilisateur a répondu avant la fin du temps imparti
     - PAS de feedback immédiat correct/incorrect par question (contrairement
       aux exercices de fin de cours) — l'utilisateur ne sait pas s'il a
       juste avant la fin de l'examen complet
   - Écran de résultat final : score global, temps total utilisé, message de
     réussite/échec (seuil ex. 70% pour réussir), et si réussi → bouton
     "Voir mon certificat" qui redirige vers la modale du certificat généré.
     Si échoué → message encourageant + bouton "Revoir les modules
     concernés" (ceux où les erreurs ont été commises) plutôt qu'un simple
     "réessayer"
